...
public class ConverterCsvOEToModel extends ConverterToModel{
        ...
        if (plStart!=null) {
            ...
            for (int i=1; i<plStart.size(); i++) {
                ...
                if (vaRecord.length>=57) {
                    ...
                    try {
                        java.util.Date vdStart = Utils.formatRelativeTimeFromBase(vaRecord[11].trim().replaceAll("\"", "").replaceAll(",", "."), 
            ...
            if (plStart != null) {
                ...
                for (int i=1; i<plStart.size(); i++) {
                    ...
                    if (vaRecord.length>=61) {
                        ...
                        //Transform the date value
                        try {
                            java.util.Date vdStart = Utils.formatRelativeTimeFromBase(vaRecord[14].trim().replaceAll("\"", "").replaceAll(",", "."), 
                                    voSta.getBaseDate(), voSta.getBaseTime());
                            voRes.setStartTime(vdStart);
                        }catch(Exception e5) {
                        }
                        ...
                        voRun.setRunnerResultList(vlRes);
                        //Now, the club of the runner
                        if (!vaRecord[21].trim().replaceAll("\"", "").equals("")) {
                            eu.oreplay.db.Club voClu = new eu.oreplay.db.Club();
                            voClu.setId("");
                            voClu.setUuid("");
    ...
    public eu.oreplay.db.Event convertResultList (File poFile) {
        eu.oreplay.db.Event voEve = null;
        if (oEve!=null) {
            List<String> plResult = new ArrayList<String>();
            BufferedReader voBr = null;
            String vcLine = "";
            try {
                //Loops the file and filters line by line
            ...
            if (plResult!=null) {
                ...
                for (int i=1; i<plResult.size(); i++) {
                    ...
                    if (vaRecord.length>=61) {
                        ...
                        ArrayList<eu.oreplay.db.ClazzControl> vlClaCon = (voCla.getClazzControlList()!=null?(ArrayList)voCla.getClazzControlList():new ArrayList<eu.oreplay.db.ClazzControl>());                    
                        //Now, the course of the class
                        ...
                        voRun.setSex((COL_SEX[vnColIndex]>=0?(vaRecord[COL_SEX[vnColIndex]].length()>0?vaRecord[11].trim().replaceAll("\"", "").charAt(0):' '):' '));
                ...
                while (i<plRun.size() && !vbFound) {
                    ...
                    if (voRun!=null) {
                        if (voRun.getRunnerResultList()!=null) {
                            if (voRun.getRunnerResultList().size()>0) {
